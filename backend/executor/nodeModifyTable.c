		   ...
		   bool canSetTag)
{
	...
	else//正常数据表
	{
		...
		if (onconflict != ONCONFLICT_NONE && resultRelInfo->ri_NumIndices > 0)
		{
			 ...
			 * 我们还没有持有任何锁，所以这不能保证以后的插入不会发生冲突。但是，如果您运行了许
			 * 多发生冲突的INSERT ON CONFLICT语句，它可以避免留下许多已取消的推测性插入。
			 *
			 * We loop back here if we find a conflict below, either during
			 * the pre-check, or when we re-check after inserting the tuple
			 * speculatively.  Better allow interrupts in case some bug makes
			 * this an infinite loop.
			 * 如果我们在下面发现冲突，无论是在预检查期间，还是在推测插入元组后重新检查时，我
			 * 们都会返回到这里。最好允许中断，以防某个错误使其成为无限循环。
			 */
	vlock:
			CHECK_FOR_INTERRUPTS();//检查是否中断
			specConflict = false;//初始化没有冲突
			 ...
			 * 唤醒所有等待我们决定的人。他们将重新检查元组，发现它不再是推测性的，并一直等待
			 * 我们的XID，就好像这是一个定期插入的元组一样。或者，如果我们杀死了元组，他们会
			 * 看到它已经死了，然后继续进行，就好像元组从未存在过一样。
			 */
			SpeculativeInsertionLockRelease(GetCurrentTransactionId());//释放锁
			 ...
			 */
			if (specConflict)//插入索引时冲突，重新检查，跳转到vlock代码块
			{
...
ExecModifyTable(PlanState *pstate)
{
	...
	/* Preload local variables *///预先构造本地变量
