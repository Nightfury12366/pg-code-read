					 ...
					 ScanDirection direction, uint64 count, bool execute_once)
{
	 ...
	 */
	operation = queryDesc->operation;//操作类型 INSERT
			...
			bool execute_once)
{
	...
	for (;;)
	{
		 ...
		 */
		slot = ExecProcNode(planstate);//执行处理单元（就是那个递归函数），这个函数返回了，下层调用链的所有函数都执行完返回了

		/*
		 * if the tuple is null, then we assume there is nothing more to
		 * process so we just end the loop...
		 */
		if (TupIsNull(slot))//如果slot返回为空，已完成处理，可以释放资源了
			break;
		...
		if (numberTuples && numberTuples == current_tuple_count)//已处理完所有tuple
			break;
	}
