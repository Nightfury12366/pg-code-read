 ...
 * the original tuple.
 *
 * heap_insert（）的子程序。为插入准备一个元组。这将设置HeapTupleFields字段，分配一个OID，
 * 并在必要时对元组进行toasts。如果是toasts元组，则返回toasts版本，如果没有，则返回原始元组。
 * 请注意，在任何情况下，HeapTupleFields也在原始元组中设置。
 */
static HeapTuple
					...
					CommandId cid, int options)
{
	...
	if (options & HEAP_INSERT_FROZEN)//冻结型插入（在事务id回卷时发生），比如删除一条数据然后回滚，就要再插一条？
		HeapTupleHeaderSetXminFrozen(tup->t_data);
    //设置cid, t_cid保存命令标识(command id, cid)，cid的意思是在当前事务中，执行当前命令之前执行了多少SQL命令
    //从0开始计数，参见PostgreSQL指南的第105页
	HeapTupleHeaderSetCmin(tup->t_data, cid);
    //设置xmax=0，t_xmax为0表示无效，等同于HEAP_XMAX_INVALID
